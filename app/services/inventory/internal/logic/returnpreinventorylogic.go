...
func (l *ReturnPreInventoryLogic) ReturnPreInventory(in *inventory.InventoryReq) (*inventory.InventoryResp, error) {
	...
	var (
		tokenItems []inventorymodel.TokenItem
	)
		...
		if ticket, err := l.svcCtx.InventoryTokenModel.CheckToken(l.ctx, in.OrderId, true); err != nil {
			...
			if errors.As(err, &tokenErr) {
				if tokenErr.Code() == "TICKET_NOT_FOUND" {
					l.Logger.Infof("return pre inventory token ticket missing: order=%d", in.OrderId)
				} else {
					l.Logger.Infof("return pre inventory token check warning: order=%d code=%s details=%v", in.OrderId, tokenErr.Code(), tokenErr.Details())
				}
			} else {
				l.Logger.Errorf("return pre inventory token check failed: order=%d err=%v", in.OrderId, err)
			}
		} else if ticket != nil {
	...
	if len(tokenItems) > 0 {
		if restored, skipped, tokenErr := l.svcCtx.InventoryTokenModel.ReturnToken(l.ctx, in.OrderId, tokenItems); tokenErr != nil {
			l.Logger.Errorf("return pre inventory token release failed: order=%d err=%v", in.OrderId, tokenErr)
		} else {
			l.Logger.Infof("return pre inventory token released: order=%d restored=%d skipped=%d", in.OrderId, restored, skipped)
		}
	}
